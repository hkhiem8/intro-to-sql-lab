...
WHERE countries.name = 'Holy See (Vatican City State)' AND isofficial = true;
...
WHERE countrylanguages.language = 'Italian' AND percentage = 100;
...
WHERE countries.name = 'San Marino' AND NOT countries.name = cities.name;
...
-- Write SQL query here
SELECT countries.name AS country, capital_cities.name AS capital
FROM countries
JOIN cities AS capital_cities ON capital_cities.id = countries.capital
JOIN cities AS known_cities ON known_cities.countrycode = countries.code
WHERE known_cities.name = 'Serra';
